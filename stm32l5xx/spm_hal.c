                  ...
                  const struct tfm_spm_partition_platform_data_t *platform_data)
{
    ...
    if (platform_data->periph_id != PERIPHERAL_NONE) {
        ppc_configure_to_secure(platform_data->periph_id);
        if (privileged) {
            ppc_clr_secure_unpriv(platform_data->periph_id);
        } else {
            ppc_en_secure_unpriv(platform_data->periph_id);
        }
    }
    return TFM_PLAT_ERR_SUCCESS;
}
...
{
    /* Clear GTZC interrupt flag */
    NVIC_ClearPendingIRQ(GTZC_IRQn);
